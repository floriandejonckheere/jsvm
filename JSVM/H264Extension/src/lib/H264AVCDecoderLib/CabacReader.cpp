...
CabacReader::CabacReader():
  ...
  m_cRefCCModel         ( 1,  NUM_FGS_REF_CTX ),
  m_cSigCCModel         ( 1,  10 ),
...
ErrVal CabacReader::isBLSkipped( MbDataAccess& rcMbDataAccess, Bool& bIsBLSkipped )
{
  UInt uiSymbol = 0;
  UInt uiCtx    = rcMbDataAccess.getCtxBLSkipFlag();

  RNOKS( CabaDecoder::getSymbol( uiSymbol, m_cBLSkipCCModel.get( 0, uiCtx ) ) );
  ...
  bIsBLSkipped = rcMbDataAccess.getMbData().getBLSkipFlag();
  return Err::m_nOK; 
}
...
ErrVal CabacReader::resPredFlag_FGS( MbDataAccess& rcMbDataAccess, Bool bBaseCoeff )
{
  ...
  RNOKS( CabaDecoder::getSymbol( uiSymbol, m_cResPredFlagCCModel.get( 0, uiCtx ) ) );
  rcMbDataAccess.getMbData().setResidualPredFlag( (uiSymbol!=0) );
                                      ...
                                      UInt&           ruiNumCoefRead )
{
  ...
  RNOKS( xRQdecodeNewTCoeffs( piCoeff, piCoeffBase, uiStop, type2ctx1[eResidualMode], type2ctx2[eResidualMode], pucScan, uiScanIndex, rbLast, ruiNumCoefRead ) );
                                     ...
                                     UInt            uiCtx )
{
  ...
  RNOKS( xRQdecodeTCoeffsRef( piCoeff, piCoeffBase, pucScan, uiScanIndex, uiCtx ) );
                                  ...
                                  UInt          uiStride )
{
  ...
  if( rbLast )
  {
    ...
    RNOKS( CabaDecoder::getSymbol( uiSymbol, m_cLastCCModel.get( uiCtx2, paiCtxEobMap[uiPrevSigCycle] ) ) );
    rbLast = (uiSymbol == 1);
    ROTRS(rbLast, Err::m_nOK);
  }
  ...
  do
  {
    ...
    {
      RNOKS( CabaDecoder::getSymbol( uiSig, m_cMapCCModel.get( uiCtx2, paiCtxSigMap[uiScanIndex] ) ) );
    } else {
      uiSig = 1;
    }
  ...
  while ( true );
    ...
    //===== SIGN =====
    RNOKS( CabaDecoder::getEpSymbol( uiSign ) );

    //===== ABSOLUTE VALUE =====
    RNOKS( CabaDecoder::getSymbol  ( uiCoeff, m_cOneCCModel.get( uiCtx1, uiCtx ) ) );

    if( uiCoeff )
    {
      uiCtx = 0;
      RNOKS( CabaDecoder::getExGolombLevel( uiCoeff, m_cAbsCCModel.get( uiCtx1, uiCtx ) ) );
      uiCoeff += 2;
    }
    else
    {
                                      ...
                                      UInt&           ruiRun )
{
      ...
      do {
        if( ! piCoeffBase[pucScan[uiScanIndex]] ) {
          if( eResidualMode == LUMA_8X8 ) {
            ...
            RNOKS( CabaDecoder::getSymbol( uiSig, m_cMapCCModel.get( type2ctx2[eResidualMode], uiScanIndex ) ) );
          }
          ruiRun ++;
        }

        uiScanIndex ++;
      } while( uiSig == 0 && uiScanIndex < uiLastScanIdx );

      if( uiSig == 0 && uiScanIndex == uiLastScanIdx ) {
        uiSig = 1;
        ruiRun ++;
      }

      // used for EOB decoding in the next cycle
      uiPrevSigCycle = uiScanIndex - 1;
    }
  }
  ...
  if( uiSig )
  {
    ...
    //===== ABSOLUTE VALUE =====
    UInt  uiCtx       = 1;
    UInt  uiType2ctx1 = ( eResidualMode == LUMA_8X8 ) ? 2 : type2ctx1[eResidualMode];
    RNOKS( CabaDecoder::getSymbol  ( uiCoeff, m_cOneCCModel.get( uiType2ctx1, uiCtx ) ) );

    if( uiCoeff ) {
      uiCtx = 0;
      RNOKS( CabaDecoder::getExGolombLevel( uiCoeff, m_cAbsCCModel.get( uiType2ctx1, uiCtx ) ) );
    ...
    riCoeff = ( uiSign ? -(Int) uiCoeff : (Int) uiCoeff );

    // the last coefficient is nonzero
    ruiRun --;
  }
...
ErrVal
CabacReader::xInitFragments   ( const SliceHeader&  rcSliceHeader,
                                ...
                                UInt                uiNumFragments )
{
  ...
  for( uiFragIdx = 1; uiFragIdx < uiNumFragments; uiFragIdx ++ )
  {
    ...
    m_apcFragmentReaders[uiFragIdx]->init( m_apcFragBitBuffers[uiFragIdx] );

    RNOK( m_apcFragmentReaders[uiFragIdx]->xInitContextModels( rcSliceHeader ) );
    RNOK( m_apcFragmentReaders[uiFragIdx]->start() );
  }
...
ErrVal
CabacReader::RQinitFragments  ( const SliceHeader&  rcSliceHeader,
                                UInt&               ruiNumFrags,
                                Bool                bCAF)
{
...
ErrVal
CabacReader::RQreleaseFragments ()
{
  UInt uiFragIdx;
  ...
  for( uiFragIdx = 1; uiFragIdx < m_uiNumFragments; uiFragIdx ++ )
  {
    ...
    if( m_apcFragmentReaders[uiFragIdx] != 0 )
    {
      delete m_apcFragmentReaders[uiFragIdx];
      m_apcFragmentReaders[uiFragIdx] = 0;
    }
  }
