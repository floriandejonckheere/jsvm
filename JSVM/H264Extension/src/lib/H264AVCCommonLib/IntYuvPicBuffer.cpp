...
UInt IntYuvPicBuffer::MSDCompute( IntYuvPicBuffer*  pcSrcYuvPicBuffer )
{
  ...
  for( y = 0; y < uiHeight; y++ )
  {
    for( x = 0; x < uiWidth; x++ )
    {
      fLumaDist += (pDes[x]-pSrc[x])*(pDes[x]-pSrc[x]);  
    }
    pSrc  += iSrcStride;
    pDes  += iDesStride;
  }
  ...
  for( y = 0; y < uiHeight; y++ )
  {
    for( x = 0; x < uiWidth; x++ )
    {
       fChromaUDist += (pDes[x]-pSrc[x])*(pDes[x]-pSrc[x]); 
    }
    pSrc  += iSrcStride;
    pDes  += iDesStride;
  }
  ...
  for( y = 0; y < uiHeight; y++ )
  {
    for( x = 0; x < uiWidth; x++ )
    {
           fChromaVDist += (pDes[x]-pSrc[x])*(pDes[x]-pSrc[x]); 
  ...
  return fLumaDist; // use only luma distorsion
}
//}}Quality level estimation and modified truncation- JVTO044 and m12007
...
ErrVal IntYuvPicBuffer::adaptiveWeighting( UShort* pusUpdateWeights, Bool bLowComplxUpdFlag )
{
  ...
  for( xb = 0; xb < uiWidth;  xb+=4 )
  {
    UShort usWeight = pusUpdateWeights[(yb*(uiWidth>>2)+xb)>>2];
  ...
  for( yb = 0; yb < uiHeight; yb+=2 )
  for( xb = 0; xb < uiWidth;  xb+=2 )
  {
    UShort usWeight = pusUpdateWeights[(yb*(uiWidth>>1)+xb)>>1];
