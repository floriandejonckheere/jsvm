...
SliceHeader::SliceHeader( const SliceHeader& rcSH )
: SliceHeaderBase   ( rcSH.getSPS(), rcSH.getPPS() )
{
  copy( rcSH );
  setPicParameterSetId( m_rcPPS.getPicParameterSetId() );
}

ErrVal
SliceHeader::copy( const SliceHeader& rcSH )
{
  *((SliceHeaderBase*)this)     = rcSH.getSliceHeaderBase();
  m_iTopFieldPoc                = rcSH.m_iTopFieldPoc;
       																  ...
       																  RefFrameList& rcRefFrameListL1 ) const
{
  ...
  {
    Int iDiffPocB = getPoc(eMbPicType) - pcFrameL0->getPoc();// TMM_INTERLACE
    Int iTDB      = gClipMinMax( iDiffPocB, -128, 127 );
    Int iTDD      = gClipMinMax( iDiffPocD, -128, 127 );
...
Bool    SliceHeader::isFieldPair ( const UInt uiFrameNum, const PicType ePicType, const Bool bIsNalRefIdc ) const
{
  // check frame num
  ROTRS( uiFrameNum != getFrameNum(), false );

 // check ref idc
  ROTRS( bIsNalRefIdc != isNalRefIdc(), false );

  // check field parity
  Int iPicType = ePicType + getPicType();
  ROTRS( FRAME!=iPicType || getPicType()==TOP_FIELD, false );
